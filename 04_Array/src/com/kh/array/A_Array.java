...
public class A_Array {
	...
	public void method1() {
		...
		// - 다량의 데이터를 기록해두고자 할 때 일일히 변수만들고 담아서 관리하면 되긴함...
//		int num1 = 0;
//		int num2 = 1;
//		int num3 = 2;
//		int num4 = 3;
...
//		arr = new int[5];
	...
	public void method3() {
		...
		// 아직 값을 대입하지 않았어도 오류없이 잘 출력이 될 수 있었던 이유가
...
//		arr[5] = 12;
		...
		// 만일 크기를 변경해야만 한다면 다시 배열을 할당(배열의 크기를 다시 지정한 새로운 배열을 만들어야됨)
		arr = new int[10];
		...
		// 끊어진 기존건 hepa영역에 둥둥 떠다니는 상태 (어느곳에도 참조되지 않고)
		// => 일정 시간이 지나면 가비지 컬렉터(GC)가 지워줌 (자바의 특징 중 하나 : 자동메모리 관리)
		
		// 지금 할당된 배열 지우고자 할 때 어떻게 해야되나요?
...
//		System.out.println("길이 : " + arr.length);
	...
	public void method7() {
		...
		System.out.println(arr == arr2);	// false	=> 주소값 비교
	...
	public void method11() {
		...
		// 0번 인덱스~마지막 인덱스까지 값 담기 (랜덤값(1~100))
		for (int i = 0; i < arr.length; i++) {
			arr[i] = (int) (Math.random() * 100 + 1);
			System.out.print(arr[i] + " ");
			if (arr[i] % 2 == 0) {
